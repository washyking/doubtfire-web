  ...
  controller: ($scope, newGroupSetService, gradeService, alertService, CsvResultModal, fileDownloaderService) ->
    ...
    $scope.addGroupSet = ->
      ...
      newGroupSetService.store(groupSet, {cache: $scope.unit.groupSetsCache}).subscribe({
        next: (gs) ->
          alertService.success( "Group set created.", 2000)
        error: (message) ->
          alertService.error( "Failed to create group set. #{message}", 6000)
      })
    ...
    $scope.saveGroupSet = (data, groupSet) ->
      ...
      newGroupSetService.update(groupSet).subscribe({
        next: (response) -> alertService.success( "Group set updated.", 2000)
        error: (message) -> alertService.error( "Failed to update group set. #{message}", 6000)
      })
    ...
    $scope.toggleLocked = (gs) ->
      ...
      newGroupSetService.update(gs).subscribe({
        next: (response) ->
          alertService.success( "#{if response.locked then 'Locked' else 'Unlocked'} #{gs.name}", 2000)
        error: (message) ->
          alertService.error( "Failed to #{if gs.locked then 'unlock' else 'lock'} #{gs.name}. #{message}", 6000)
      })
    ...
    $scope.removeGroupSet = (gs) ->
      newGroupSetService.delete(gs, {cache: $scope.unit.groupSetsCache}).subscribe({
        next: (response) ->
          ...
          alertService.success( "Group set deleted.", 2000)
        error: (message) -> alertService.error( "Failed to delete group set. #{message}", 6000)
      })
