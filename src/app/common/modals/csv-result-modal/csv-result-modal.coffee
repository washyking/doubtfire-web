...
.factory("CsvResultModal", ($modal, alertService) ->
  ...
  CsvResultModal.show = (title, response) ->
    if response.errors.length == 0
      alertService.success( "Data uploaded. Success with #{response.success.length} items.", 2000)
    else if response.success.length > 0
      alertService.message( "Data uploaded, success with #{response.success.length} items, but #{response.errors.length} errors.", 6000)
    else
      alertService.error( "Data uploaded but #{response.errors.length} errors", 6000)
